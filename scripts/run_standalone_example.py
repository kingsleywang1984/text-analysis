...
ROOT_DIR = Path(__file__).resolve().parents[1]
DATA_FILE = ROOT_DIR / "data" / "input_example.json"
...
def main() -> None:
    try:
        ...
        # For local demo convenience: if no provider configured, use FakeLLMClient to show LLM-shaped output.
        llm = create_llm_client(cfg) or FakeLLMClient()

        resp = analyze_request(req, cfg, llm=llm)

        # Print the full response JSON (matches expected standalone/comparison output schema).
        print(resp.model_dump_json(indent=2, ensure_ascii=False))
    except Exception as e:
        print("ERROR running standalone example:", file=sys.stderr)
